let peliculas = [
  ...
  const mayorYMenor = () => {
    ...
    peliculas.forEach ((peli) => {
      if (peli.vote_count > mayor) {
        ...
        peliMayor = peli.original_title;
      } ;
      if (peli.vote_count < menor) {
        menor = peli.vote_count;
        peliMenor = peli.original_title;
      } ;
    })
    console.log(`Pelicula con mayor cantidad de votos : ${peliMayor} (${mayor} votos)`);
    console.log(`Pelicula con menor cantidad de votos : ${peliMenor} (${menor} votos)`);
  }
  mayorYMenor();
  ...
  const mayorCantidadDeGeneros = () => {
    ...
    const pelisMayor = peliculas.filter( (peli) => {
      if (peli.genre_ids.length == mayor) {
        return peli;
      }
    })
    console.log(pelisMayor)
  }
  ...
  const peliculasConGeneros = () => {
    const nuevoPeliculas = peliculas.map((peli)=>{
      peli.generos = [];
      peli.genre_ids.forEach((id) => {
        const genero = generos.find((gen) => {
          if (gen.id === id) {
            peli.generos.push(gen.name);
          }
        });
      })
      return peli;
