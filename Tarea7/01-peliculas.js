let peliculas = [
  ...
  let tarjetas = document.getElementById("main__peliculas");
  let aside = document.getElementById("main__categorias");
  ...
  const dibujarTarjetas = () => {
    peliculas.forEach((peli) => {
      ...
      peli.generos.forEach((gen) => {
        botonesGeneros = botonesGeneros + `<button class="main__genero">${gen.toUpperCase()}</button> `;
      });
      let card = document.createElement("div");
      card.classList.add("main__pelicula");
      card.innerHTML = `<div class="main__ilustracion">
      <img src="https://image.tmdb.org/t/p/w500${peli.poster_path}" alt="">
      </div>
      <div class="main__texto">
      <h3 class="main__titulo">${peli.title}</h3>
      <span class="main__fecha">${peli.release_date}</span>
      <p class="main__sinopsis">
      ${peli.overview}
      </p>
      <div class="main__generos">
      ${botonesGeneros}
      </div>
      </div>`;
  ...
  const dibujarCategorias = () => {
    generos.forEach((gen) => {
      ...
      div.innerText = gen.name.toUpperCase();
      div.classList.add("main__categoria");
